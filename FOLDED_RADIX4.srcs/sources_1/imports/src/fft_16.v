...
module fft_16_radix4 (
    ...
    reg stage; // Only 2 stages: 0 and 1
    reg [2:0] state;
    reg [2:0] wait_count;
    
    localparam IDLE = 0, BIT_REVERSE = 1, WAIT_STAGE = 2, WRITE_STAGE = 3, DONE = 4;
    
    // Instantiate 4 butterflies in parallel for radix-4 folded architecture
    reg signed [15:0] bf_a_re[0:3];
    reg signed [15:0] bf_a_im[0:3];
    reg signed [15:0] bf_b_re[0:3];
    reg signed [15:0] bf_b_im[0:3];
    reg signed [15:0] bf_c_re[0:3];
    reg signed [15:0] bf_c_im[0:3];
    reg signed [15:0] bf_d_re[0:3];
    reg signed [15:0] bf_d_im[0:3];
    
    reg signed [15:0] bf_w1_re[0:3];
    reg signed [15:0] bf_w1_im[0:3];
    reg signed [15:0] bf_w2_re[0:3];
    reg signed [15:0] bf_w2_im[0:3];
    reg signed [15:0] bf_w3_re[0:3];
    reg signed [15:0] bf_w3_im[0:3];
    
    wire signed [15:0] bf_y1_re[0:3];
    wire signed [15:0] bf_y1_im[0:3];
    wire signed [15:0] bf_y2_re[0:3];
    wire signed [15:0] bf_y2_im[0:3];
    wire signed [15:0] bf_y3_re[0:3];
    wire signed [15:0] bf_y3_im[0:3];
    wire signed [15:0] bf_y4_re[0:3];
    wire signed [15:0] bf_y4_im[0:3];
    ...
    always @(*) begin
        for(b=0; b<4; b=b+1) begin
            if(stage == 0) begin // Groups of 4, spacing 1
                ...
                // Twiddles are W^0 for all in stage 0
            ...
            end else begin // stage 1: Group of 16, spacing 4
                bf_a_re[b] = data_re[b];
                bf_a_im[b] = data_im[b];
    ...
    integer j;
    // Base-4 digit reversal for 16 points
    wire [3:0] rev [0:15];
    ...
    // Same sample test signal from Radix-2
    ...
    always @(posedge clk) begin
        if (rst) begin
            ...
            case(state)
                ...
                WAIT_STAGE: begin
                    // Radix-4 butterfly latency is 6 cycles
